...
func (rp *ReportHandler) GetEmployeeSalaryReport(w http.ResponseWriter, r *http.Request) {
	...
	// 4. Fetch Report (Data + Count + Totals)
	branchReport, err := rp.DB.GetEmployeeSalaryReport(r.Context(), branchID, startDate, endDate, reportType, page, limit, search)
	if err != nil {
		rp.errorLog.Println("ERROR_02_GetEmployeeSalaryReport: ", err)
		utils.BadRequest(w, err)
		return
	}
