...
/* --- INITIALIZATION --- */
window.initStockReportPage = async function () {
  // 1. Grab Elements
...
async function fetchReport() {
  ...
  try {
    const response = await fetch(
      `${window.globalState.apiBase}/products/stocks?${params.toString()}`,
      {
        method: "GET",
...
window.printReport = function () {
  ...
  const columns = [
    { label: "Date", key: "stock_date", align: "left", action:"date" },
    { label: "Product", key: "product_name", align: "center" },
    { label: "Memo No", key: "memo_no", align: "center" },
