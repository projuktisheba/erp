...
async function fetchReport() {
  ...
  try {
    const response = await fetch(
      `${window.globalState.apiBase}/reports/employee/progress?${params.toString()}`,
      {
        method: "GET",
...
function renderReportTable() {
  ...
  reportState.data.forEach((row) => {
    tbody.innerHTML += `
            <tr class="hover:bg-slate-50 border-b border-slate-50 transition text-slate-700">
                <td class="px-4 py-3 border-r border-slate-100 font-medium">${formatDateVal(row.sheet_date)}</td>
                <td class="px-4 py-3 text-center border-r border-slate-100">${row.sales_person_name || "Unknown"}</td>
                <td class="px-4 py-3 text-center border-r border-slate-100">${row.mobile || "-"}</td>
                
                <td class="px-4 py-3 text-right text-purple-700 border-r border-slate-100">
                    ${row.order_count}
                </td>
  ...
  tfoot.innerHTML = `
        <tr class="bg-slate-100 border-t-2 border-slate-200">
            ...
            <td class="px-4 py-3 text-right">${t.sale}</td>
            <td class="px-4 py-3 text-right">${t.sale_return}</td>
        </tr>
    `;
}
...
window.printReport = function () {
  ...
  const columns = [
    ...
    { label: "Salesperson Name", key: "sales_person_name", align: "left" },
    { label: "Mobile", key: "mobile", align: "left" },
    { label: "Order Count", key: "order_count", align: "right" },
    { label: "Sale", key: "sale", align: "right" },
    { label: "Sale Return", key: "sale_return", align: "right" },
  ];
