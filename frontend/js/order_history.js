...
async function fetchOrders() {
  ...
  if (orderHistoryState.branchID) params.append("branch_id", orderHistoryState.branchID);
  if (orderHistoryState.searchQuery) params.append("search", orderHistoryState.searchQuery);
  if (orderHistoryState.statusFilter && orderHistoryState.statusFilter !== "all") {
...
function renderOrders() {
  ...
  // Render Rows
  tbody.innerHTML = orderHistoryState.orders.map((o) => {
      ...
      const dateStr = new Date(o.order_date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" });

      // Status Styling
      let st = { css: "bg-slate-100 text-slate-600 border-slate-200", dot: "bg-slate-400" };
      if (o.status === "delivered") st = { css: "bg-emerald-50 text-emerald-700 border-emerald-200", dot: "bg-emerald-500" };
      else if (o.status === "pending") st = { css: "bg-amber-50 text-amber-700 border-amber-200", dot: "bg-amber-500" };
      else if (o.status === "partial") st = { css: "bg-blue-50 text-blue-700 border-blue-200", dot: "bg-blue-500" };
      else if (o.status === "cancelled") st = { css: "bg-rose-50 text-rose-700 border-rose-200", dot: "bg-rose-500" };
      ...
      <tr class="group border-b border-slate-100 last:border-0 hover:bg-slate-50/80 transition-all duration-200">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
          <div class="flex items-center">
            <div class="h-10 w-10 rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold ${getAvatarColor(customerName)} mr-3 ring-1 ring-slate-100">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-center">
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${st.css}">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-center">
          <div class="flex items-center justify-center space-x-3 opacity-80 group-hover:opacity-100 transition-opacity">
            <button onclick="viewOrder(${o.id})" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-white rounded-lg transition-all border border-transparent hover:border-slate-200 hover:shadow-sm" title="View">
...
window.deliverOrder = function(id) {
    ...
    // Trigger your Delivery Logic here
};
...
async function viewOrder(id) {
    ...
    if(itemsBody) itemsBody.innerHTML = `<tr><td colspan="3" class="px-4 py-8 text-center text-sm text-slate-500 animate-pulse">Loading items...</td></tr>`;
    if(transBody) transBody.innerHTML = `<tr><td colspan="4" class="px-4 py-8 text-center text-sm text-slate-500 animate-pulse">Loading history...</td></tr>`;

    try {
        const response = await fetch(`${window.globalState.apiBase}/products/orders/${id}`);
        ...
        if (statusEl) {
            ...
            // Reset base classes
            statusEl.className = "inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ring-1 ring-inset uppercase tracking-wide";
            
            // Apply specific color schemes
            if(status === 'delivered') {
                statusEl.classList.add("bg-emerald-50", "text-emerald-700", "ring-emerald-600/20");
            } else if(status === 'pending') {
                statusEl.classList.add("bg-amber-50", "text-amber-700", "ring-amber-600/20");
            } else if(status === 'partial') {
                statusEl.classList.add("bg-blue-50", "text-blue-700", "ring-blue-600/20");
            } else if(status === 'cancelled') {
                statusEl.classList.add("bg-rose-50", "text-rose-700", "ring-rose-600/20");
            } else {
                statusEl.classList.add("bg-slate-50", "text-slate-600", "ring-slate-500/20");
            }
        }
        ...
        if (transBody) {
            if (transactions.length === 0) {
                transBody.innerHTML = `<tr><td colspan="4" class="px-4 py-6 text-center text-sm text-slate-400 italic">No history available.</td></tr>`;
            } else {
                transBody.innerHTML = transactions.map(t => {
                    const typeStr = t.transaction_type.toLowerCase();
                    const isPayment = typeStr.includes('payment') || typeStr.includes('advance');
                    const isDelivery = typeStr.includes('delivery');

                    let typeBadgeClass = "bg-slate-100 text-slate-600 border-slate-200"; // Default
                    if (isPayment) typeBadgeClass = "bg-emerald-50 text-emerald-600 border-emerald-200";
                    if (isDelivery) typeBadgeClass = "bg-blue-50 text-blue-600 border-blue-200";
        ...
        if(itemsBody) itemsBody.innerHTML = `<tr><td colspan="3" class="px-4 py-4 text-center text-red-500">Error loading data.</td></tr>`;
    }
}

window.printSelectedOrder = async function () {
    printOrderInvoice(orderHistoryState.selectedOrder.id, orderHistoryState.selectedOrder)
}
